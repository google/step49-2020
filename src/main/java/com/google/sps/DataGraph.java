...
abstract class DataGraph {
  ...
  boolean graphFromProtoNodes(Map<String, Node> protoNodesMap) {
    ...
    for (String nodeName : protoNodesMap.keySet()) {
      ...
      for (String tokenName : thisNode.getTokenList()) {
        ...
        Set<String> nodesWithToken = this.tokenMap().containsKey(tokenName) ? this.tokenMap().get(tokenName)
            : new HashSet<>();
   ...
   *
   * @param names  the names of the nodes to search for & include in the graph.
   *               using a collection for flexibility.
