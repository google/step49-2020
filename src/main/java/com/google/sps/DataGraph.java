...
abstract class DataGraph {
  ...
  /**
   * <<<<<<< HEAD Returns a MutableGraph with nodes that are at most a certain radius from a given
   * nodes. If the radius is less than 0 or the node specified isn't present, return an empty graph.
   ...
   * result of the other nodes. ======= Returns a MutableGraph with nodes that are at most a certain
   * radius from a given nodes. If the radius is less than 0 or the node specified isn't present,
   * return an empty graph. Since maxDepth was implemented as DFS, we use BFS for *diversity*.
