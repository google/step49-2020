...
public class MutationDiffTest {
    ...
    public void nonConsecutiveIndicesPos() {
        ...
        Mutation addAC = Mutation.newBuilder().setType(Mutation.Type.ADD_EDGE).setStartNode("A").setEndNode("C")
                .build();
    ...
    public void nonConsecutiveIndicesNeg() {
        ...
        Mutation addAD = Mutation.newBuilder().setType(Mutation.Type.ADD_EDGE).setStartNode("A").setEndNode("D")
                .build();
    ...
    public void backwardMutationDeleteTokens() {
        ...
        TokenMutation tokenMut = TokenMutation.newBuilder().setType(TokenMutation.Type.DELETE_TOKEN).addTokenName("1")
                .addTokenName("2").addTokenName("3").build();
    ...
    public void backwardMutationAddSyntheticNode() {
        ...
        TokenMutation tokenMutRem = TokenMutation.newBuilder().setType(TokenMutation.Type.DELETE_TOKEN)
                .addTokenName("1").build();

        Mutation removeTokenFromA = Mutation.newBuilder().setStartNode("A").setType(Mutation.Type.CHANGE_TOKEN)
                .setTokenChange(tokenMutRem).build();

        Mutation removeTokenFromB = Mutation.newBuilder().setStartNode("B").setType(Mutation.Type.CHANGE_TOKEN)
                .setTokenChange(tokenMutRem).build();

        TokenMutation tokenMutAdd = TokenMutation.newBuilder().setType(TokenMutation.Type.ADD_TOKEN).addTokenName("1")
                .build();

        Mutation addTokenToC = Mutation.newBuilder().setStartNode("C").setType(Mutation.Type.CHANGE_TOKEN)
                .setTokenChange(tokenMutAdd).build();

        Mutation addCA = Mutation.newBuilder().setType(Mutation.Type.ADD_EDGE).setStartNode("C").setEndNode("A")
                .build();
        Mutation addCB = Mutation.newBuilder().setType(Mutation.Type.ADD_EDGE).setStartNode("C").setEndNode("B")
                .build();

        MultiMutation synthModCM = MultiMutation.newBuilder().addMutation(addC).addMutation(removeTokenFromA).addMutation(removeTokenFromB).addMutation(addTokenToC).addMutation(addCA).addMutation(addCB)
                .setReason("extracting common information from A and B into synthetic module C").build();
