...
public final class Utility {
  ...
  public static MultiMutation diffBetween(
    ...
    if (currIndex < 0
      ...
      MultiMutation currMultiMut = multiMutList.get(currIndex);
      // List<Mutation> currMutList = currMultiMut.getMutationList();

      // List<Mutation> resultMutList = new ArrayList<>();
      // resultMutList.addAll(currMutList);
      // Collections.reverse(resultMutList);

      // MultiMutation resultMultiMut = MultiMutation.newBuilder().setReason(currMultiMut.getReason()).addAllMutation(resultMutList).build();
